...
namespace AuthAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        {
            var users = await _userManager.Users.ToListAsync(); // Primero carga los usuarios
            var userDtos = new List<UserDetailDto>();

            foreach (var user in users)
            {
                var roles = await _userManager.GetRolesAsync(user); // Usa await para cada operación
